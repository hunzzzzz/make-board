...
CREATE
OR REPLACE
   TRIGGER trg_board_post_id BEFORE INSERT ON t_board_post FOR EACH ROW BEGIN :NEW.id := seq_t_board_post.NEXTVAL;
END;
/ -- 테이블 생성
...
(
   ...
   CONSTRAINT fk_post_user FOREIGN KEY (user_id) REFERENCES t_board_user (id) ON DELETE CASCADE,
